    ...
    public class Peer
    {
        ...
        public void Connect()
        {
            if (TcpClient == null)
            {
                ...
                {
                    Log.WriteLine(e.ToString(), EventLogEntryType.Error);
                    Disconnect();
                    return;
                }
            }
        ...
        private void HandleRead(IAsyncResult ar)
        {
            ...
            try
            {
                Stream.BeginRead(streamBuffer, 0, bufferSize, new AsyncCallback(HandleRead), null);
            }
            catch (Exception e)
            {
                Log.WriteLine(e.ToString(), EventLogEntryType.Error);
        ...
        private void HandleMessage(byte[] bytes)
        {
            ...
            {
                if (DecodeHandshake(bytes, out byte[] hash, out string id))
                {
            ...
            {
                if (DecodeHave(bytes, out int index))
                {
                    HandleHave(index);
                    return;
                }
            ...
            {
                if (DecodeBitfield(bytes, IsPieceDownloaded.Length, out bool[] isPieceDownloaded))
                {
            ...
            {
                if (DecodeRequest(bytes, out int index, out int begin, out int length))
                {
            ...
            {
                if (DecodePiece(bytes, out int index, out int begin, out byte[] data))
                {
            ...
            {
                if (DecodeCancel(bytes, out int index, out int begin, out int length))
                {
