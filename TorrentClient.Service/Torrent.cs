...
namespace TorrentClient.Service
{
    public class Torrent
    {
        ...
        public string HexStringInfohash { get { return string.Join("", Infohash.Select(x => x.ToString("x2"))); } }
        ...
        {
            PeerListUpdated?.Invoke(sender, endPoints);
        }
        ...
        {
            object obj = BEncode.DecodeFile(filePath);
        ...
        private static object TorrentInfoToBEncodingObject(Torrent torrent)
        {
            Dictionary<string, object> dict = new Dictionary<string, object>
            {
                ["piece length"] = (long)torrent.PieceSize
            };
            ...
            else
            {
                ...
                foreach (var f in torrent.Files)
                {
                    ...
                    {
                        ["path"] = f.Path.Split(Path.DirectorySeparatorChar).Select(x => (object)Encoding.UTF8.GetBytes(x)).ToList(),
                        ["length"] = f.Size
                    };
                    files.Add(fileDict);
                }
                ...
                dict["files"] = files;
                dict["name"] = Encoding.UTF8.GetBytes(torrent.FileDirectory[0..^1]);
            }
        ...
        private static Torrent BEncodingObjectToTorrent(object bencoding, string name, string downloadPath)
        {
            ...
            if (obj == null)
            {
                var ex = new Exception("not a torrent file");
                Log.LogException(ex);
                throw ex;
            }
            ...
            {
                var ex = new Exception("Missing info section");
                Log.LogException(ex);
            }
            ...
            else if (info.ContainsKey("files"))
            {
                ...
                foreach (object item in (List<object>)info["files"])
                {
                    ...
                    if (!dict.ContainsKey("path") || !dict.ContainsKey("length"))
                    {
                        var ex = new Exception("error: incorrect file specification");
                        Log.LogException(ex);
                        throw ex;
                    }
            ...
            {
                var ex = new Exception("error: no files specified in torrent");
                Log.LogException(ex);
                throw ex;
            }
            ...
            {
                var ex = new Exception("No piece length key in info section of the torrent file");
                Log.LogException(ex);
                throw ex;
            }
            ...
            {
                var ex = new Exception("No pieces key in info section of the torrent file");
                Log.LogException(ex);
                throw ex;
            }
        ...
        public static string DecodeUTF8String(object obj)
        {
            ...
            {
                var ex = new Exception("unable to decode utf-8 string, object is not a byte array");
                Log.LogException(ex);
                throw ex;
            }
            else
