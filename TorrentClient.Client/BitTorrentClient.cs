...
namespace TorrentClient.Client
{
    public class BitTorrentClient
    {
        ...
        private static readonly int maxSeeders = 5;

        private static readonly int maxUploadBytesPerSecond = 16384;
        private static readonly int maxDownloadBytesPerSecond = 16384;
        ...
        private readonly ConcurrentQueue<DataRequest> OutgoingBlocks = new ConcurrentQueue<DataRequest>();
        private readonly ConcurrentQueue<DataPackage> IncomingBlocks = new ConcurrentQueue<DataPackage>();
        ...
        public BitTorrentClient(int port, string torrentPath, string downloadPath)
        {
        ...
        private void HandleNewConnection(IAsyncResult ar)
        {
            ...
            {
                Log.WriteLine("connection to new peer failed", EventLogEntryType.Warning);
                return;
            }
        ...
        private void HandlePeerDisconnected(object sender, EventArgs args)
        {
            ...
            Peers.TryRemove(peer.Key, out Peer tmp);
        ...
        private void ProcessUploads()
        {
            ...
            while (!uploadThrottle.IsThrottled && OutgoingBlocks.TryDequeue(out DataRequest block))
            {
        ...
        private void ProcessDownloads()
        {
            ...
            while (IncomingBlocks.TryDequeue(out DataPackage incomingBlock))
